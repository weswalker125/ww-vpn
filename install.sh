...
sed -i -e '/%CRT%/{r ./pki/issued/server.crt' -e 'd}' /etc/openvpn/server.conf
sed -i -e '/%KEY%/{r ./pki/private/server.key' -e 'd}' /etc/openvpn/server.conf
...
for client in "${CLIENTS[@]}"; do
  ...
  #didn't work:
  cp ./pki/issued/${client}.crt ./pki/issued/current.crt
  cp ./pki/private/${client}.key ./pki/private/current.key
